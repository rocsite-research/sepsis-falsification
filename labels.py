...
def compute_sofa(
    ...
    # motor (184,223901)
    # GCS itemids: eye (723, 220739), verbal (454, 223900), motor (184, 223901)
    GCS_EYE_ITEMS    = [723, 220739]
    GCS_VERBAL_ITEMS = [454, 223900]
    GCS_MOTOR_ITEMS  = [184, 223901]
    GCS_ITEMIDS = GCS_EYE_ITEMS + GCS_VERBAL_ITEMS + GCS_MOTOR_ITEMS
    try:
        charts = pd.read_csv(
        ...
        charts["component"] = "motor"
        charts.loc[charts["itemid"].isin(GCS_EYE_ITEMS),    "component"] = "eye"
        charts.loc[charts["itemid"].isin(GCS_VERBAL_ITEMS), "component"] = "verbal"

        # Use merge_asof with 5-minute tolerance to pair GCS components
        # that are charted at slightly different timestamps (seconds/minutes apart).
        # This matches the approach used for PaO2/FiO2 respiratory component.
        # Fix for: (1) charttime was not loaded in usecols causing silent sofa_cns=0
        # for all stays; (2) exact timestamp grouping failed to pair components
        # charted at slightly different times. (v2 correction)
        eye    = charts[charts["component"] == "eye"   ][["stay_id","charttime","valuenum"]].rename(columns={"valuenum":"eye"})
        verbal = charts[charts["component"] == "verbal"][["stay_id","charttime","valuenum"]].rename(columns={"valuenum":"verbal"})
        motor  = charts[charts["component"] == "motor" ][["stay_id","charttime","valuenum"]].rename(columns={"valuenum":"motor"})

        eye    = eye.sort_values(["stay_id","charttime"])
        verbal = verbal.sort_values(["stay_id","charttime"])
        motor  = motor.sort_values(["stay_id","charttime"])

        # Merge eye + verbal within 5 minutes
        ev = pd.merge_asof(
            eye, verbal,
            on="charttime", by="stay_id",
            tolerance=pd.Timedelta("5min"),
            direction="nearest",
        )
        # Merge result + motor within 5 minutes
        evm = pd.merge_asof(
            ev.sort_values(["stay_id","charttime"]),
            motor,
            on="charttime", by="stay_id",
            tolerance=pd.Timedelta("5min"),
            direction="nearest",
        )
        evm = evm.dropna(subset=["eye","verbal","motor"])
        evm["gcs_total"] = (evm["eye"] + evm["verbal"] + evm["motor"]).clip(3, 15)

        gcs_min = (
            evm.groupby("stay_id")["gcs_total"]
            .min()
            .reset_index()
        )
